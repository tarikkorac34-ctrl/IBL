...
// In-Memory Standings Data (1 Division)
let standingsData = TEAMS.map(team => ({
  name: team.name,
  played: 0,
  won: 0,
  drawn: 0,
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(opt => opt.setName('time').setDescription('Match Time/Date (e.g. 20:00 UTC)').setRequired(true))
    .addStringOption(opt => 
      opt.setName('stadium')
  ...
  new SlashCommandBuilder()
    .setName('standings')
    .setDescription('View current IBL League Standings'),
...
client.on('interactionCreate', async (interaction) => {
  ...
  // ------------------------------------------------------------------
  // COMMAND: /signingrequest & /signingrelease
  // ------------------------------------------------------------------
  ...
  if (commandName === 'game') {
    ...
    setTimeout(() => {
      targetChannel.send({ content: `@here The match between **${home}** and **${away}** starts in 30 minutes!` });
    }, 5000); 
  ...
  if (commandName === 'results') {
    ...
    const titleHeader = isCup 
      ? `${EMOJIS.CUP} | Match Result` 
      : `${EMOJIS.IBL} | Match Result`;
    ...
    if (!isCup) {
      updateStandings(home, away, homeScore, awayScore);
    }

    return interaction.reply({ content: `Result sent to #${targetChannelName}!`, ephemeral: true });
  }
  ...
  if (commandName === 'standings') {
    ...
    try {
      ...
      const bgPath = path.join(__dirname, 'stadium.png');
      const background = await loadImage(bgPath);

      ctx.drawImage(background, 0, 0, canvas.width, canvas.height);

      ctx.fillStyle = 'rgba(15, 23, 42, 0.85)';
      ctx.fillRect(40, 40, 920, 420);
      ...
      standingsData.sort((a, b) => b.points - a.points || b.gd - a.gd);
      ...
      console.error(err);
      return interaction.editReply({ content: 'Failed to generate standings table image. Check if stadium.png exists.' });
    }
  }
});

function updateStandings(homeName, awayName, homeScore, awayScore) {
  const home = standingsData.find(t => t.name.toLowerCase().includes(homeName.toLowerCase()));
  const away = standingsData.find(t => t.name.toLowerCase().includes(awayName.toLowerCase()));
