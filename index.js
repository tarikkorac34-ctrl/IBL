...
// ==========================================
// 1. HTTP SERVER (Keeps bot online on Render/hosting)
// ==========================================
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    if (!customId.startsWith('req_')) return;
    ...
    // Custom ID pattern: req_ACTION_TYPE_TARGETID_REQUESTERID_SEASON_TEAMNAME
    const parts = customId.split('_');
    const action = parts[1];      // 'accept' or 'decline'
    const type = parts[2];        // 'signing' or 'release'
    const targetId = parts[3];
    const requesterId = parts[4];
    const seasonVal = parts[5];
    const teamName = parts.slice(6).join('_');
    ...
    if (isAccept && isSigning && targetUser) {
      ...
      const robloxField = origEmbed.fields ? origEmbed.fields.find(f => f.name === 'Roblox Username') : null;
    ...
    if (requester) {
      ...
      if (isSigning) {
        dmMsg = isAccept 
          ? `Player **<@${targetId}>** has been **accepted** to play for **${teamName}**!`
          : `Player **<@${targetId}>** has been **declined** to play for **${teamName}**.`;
      } else {
        dmMsg = isAccept
          ? `Player **<@${targetId}>** has been **released** from **${teamName}**.`
          : `Player **<@${targetId}>** has **not been released** from **${teamName}**.`;
      }
  ...
  if (commandName === 'requestsigning' || commandName === 'requestrelease') {
    ...
    const actionType = isSigning ? 'signing' : 'release';

    const buttons = new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`req_accept_${actionType}_${player.id}_${user.id}_${season}_${team}`)
        .setLabel('Accept')
        .setStyle(ButtonStyle.Success),
      new ButtonBuilder()
        .setCustomId(`req_decline_${actionType}_${player.id}_${user.id}_${season}_${team}`)
        .setLabel('Decline')
  ...
  // Games, Results, Teams, Standings handlers stay fully intact
  if (commandName === 'game') {
    if (!hasRolePermission(member, STAFF_ROLES)) {
      return interaction.reply({ content: '❌ You do not have permission to use this command.', ephemeral: true });
    }
    ...
    try {
      const scheduledStartTime = new Date(Date.now() + 3600000);
