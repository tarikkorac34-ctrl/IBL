...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName('results')
    .setDescription('Post match results and update MongoDB database')
    .addStringOption(opt => 
      opt.setName('type')
        .setDescription('Match Type')
        .setRequired(true)
...
client.on('interactionCreate', async (interaction) => {
  ...
  // Roles allowed for Staff commands (/game, /results)
  const staffRoles = ['REGISTRATION STAFF'];

  // Roles allowed for Management commands (/requestsigning, /requestrelease, /refereerequest)
  const managementRoles = ['REGISTRATION STAFF', 'IBL | Team Owner', 'IBL | Team Manager'];

  // ------------------------------------------------------------------
  // /refereerequest
  // ------------------------------------------------------------------
  if (commandName === 'refereerequest') {
    if (!hasRolePermission(member, managementRoles)) {
  ...
  if (commandName === 'requestsigning' || commandName === 'requestrelease') {
    ...
    const isRequest = commandName === 'requestsigning';
  ...
  // ------------------------------------------------------------------
  if (commandName === 'game') {
    if (!hasRolePermission(member, staffRoles)) {
      return interaction.reply({ content: '❌ You do not have permission to use this command.', ephemeral: true });
    }
  ...
  if (commandName === 'results') {
    ...
    return interaction.reply({ content: `Result saved to database & posted to #${targetChannelName}!`, ephemeral: true });
  }
